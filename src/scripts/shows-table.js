...
function getTypeLabel(categoryArray) {
  ...
  if (catSet.has("video") && catSet.size === 1) {
    return '<span class="type-label video-label" data-testid="type-label" data-kind="video">v</span>';
  } else if (catSet.has("video") && catSet.has("misc")) {
    return '<span class="type-label misc-label" data-testid="type-label" data-kind="misc">m</span>';
  } else if (catSet.has("video") && catSet.has("compilation")) {
    return '<span class="type-label video-label" data-testid="type-label" data-kind="video">v</span>';
  } else if (catSet.has("audio") && catSet.size === 1) {
    return '<span class="type-label audio-label" data-testid="type-label" data-kind="audio">a</span>';
    ...
    <td>${
      show.startDateUnix
        ...
        ? show.startDate.day && show.startDate.month && show.startDate.year
          ? `${show.startDate.year}-${String(show.startDate.month).padStart(
              2,
              "0"
            )}-${String(show.startDate.day).padStart(2, "0")}`
...
<td class="size-cell">
  <div class="media-wrapper">
    <span>${smartSize(show.specs.media[0].size)}${
      show.specs.media[0].unit
    }</span>
    ${
      show.specs.media.length > 1
        ? `<button type="button" class="count-badge toggle-media"
                 data-target="media-${i}"
                 aria-expanded="false"
                 title="Show ${show.specs.media.length - 1} more size${
            show.specs.media.length - 1 === 1 ? "" : "s"
          }">
           +${show.specs.media.length - 1}
         </button>`
        : ""
  ...
  ${
    show.specs.media.length > 1
      ? `<div id="media-${i}" class="extra-media">
         ${show.specs.media
           .slice(1)
           .map((m) => `<div>${smartSize(m.size)}${m.unit}</div>`)
           .join("")}
       </div>`
      : ""
  }
...
<td class="type-cell">
    ...
    <td class="type-cell">${show.source || "—"}
        ${
          show.fileSlug?.includes("show_1")
            ? '<span class="show-label">1</span>'
            : ""
        }
        ${
          show.fileSlug?.includes("show_2")
            ? '<span class="show-label">2</span>'
            : ""
        }
        ${
          show.master === true ||
          (show.tapers?.length === 1 && show.tapers[0] === "Vortex")
            ? '<span class="trade-label master">M</span>'
            : ""
        }
    </td>
    <td class="type-cell taper-cell">${
      show.tapers?.length
        ? `
  <div class="media-wrapper">
    ...
    ${
      show.tapers.length > 1
        ? `<button type="button"
                 ...
                 aria-expanded="false"
                 title="Show ${show.tapers.length - 1} more taper${
            show.tapers.length - 1 === 1 ? "" : "s"
          }">
            +${show.tapers.length - 1}
         </button>`
        : ""
...
`
    ...
    <td class="page-cell">
      <a href="${pathPrefix}/shows/${
      show.fileSlug
    }/" target="_blank" rel="noopener noreferrer" style="font-size: 1.20em;">🎫</a>
...
function insertGroupLabels(selectedBands) {
  ...
  const visible = rows.filter(
    (r) => r.style.display !== "none" && !r.hasAttribute("data-label")
  );
  ...
  for (const row of visible) {
    ...
    if (Array.isArray(catArr)) {
      ...
      if (set.has("video") && set.size === 1) {
        ...
        (set.has("video") && set.has("misc")) ||
        (set.has("misc") && set.size === 1)
      ) {
        catKey = "video_misc";
      } else if (
        (set.has("video") && set.has("compilation")) ||
        (set.has("compilation") && set.size === 1)
      ) {
...
function initializeShowFilters(shows) {
  ...
  function buildYearDropdown() {
    ...
    const singleBand =
      currentFilterBands.length === 1 ? currentFilterBands[0] : null;
    if (!singleBand) {
      menu.innerHTML = "";
      return;
    }
    ...
    let html = `
        <li><a class="dropdown-item ${
          currentFilterYear === "all" ? "active" : ""
        }" href="#" data-year="all">All years</a></li>
    ...
    html += years
      .map(
        (y) =>
          `<li><a class="dropdown-item ${
            currentFilterYear === String(y) ? "active" : ""
          }" href="#" data-year="${y}">${y}</a></li>`
      )
      .join("");
  ...
  function renderPage(filteredShows, page) {
    ...
    if (showsForPage.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="13" style="text-align: center;">No shows match your filters.</td></tr>';
      return;
    }
...
`;
  ...
  function filterShows() {
    ...
    let filtered = shows.filter((show) => {
      ...
      const bandMatch = show.bands.some((b) =>
        lowerCaseBands.includes(b.toLowerCase())
      );
      return letterMatch && bandMatch;
    });

    const pillMode =
      currentFilterLetter !== "all" && currentFilterBands.length > 0;

    if (currentFilterLetter === "all") {
      filtered.sort((a, b) => {
        const unixA = a.startDateUnix,
          unixB = b.startDateUnix;
  ...
  function buildBandPills() {
    ...
    container.querySelectorAll(".band-pill").forEach((pill) => {
      pill.addEventListener("click", () => {
        ...
        currentFilterBands = [
          ...document.querySelectorAll(".band-pill.bg-primary"),
        ].map((p) => p.dataset.band);
