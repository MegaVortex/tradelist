...
async function parseTitles() {
    const input = fs.readFileSync('site.txt', 'utf8');
    ...
    for (const match of allMatches) {
        ...
        title = title.replace('[Compilation]', '').trim();
        ...
        } else if ((!categoryList.includes('video'))) {
        ...
        const categoriesForFilename = (
            categoryList.includes('audio')
                ? (categoryList.includes('misc') ? 'audio_misc' : 'audio')
                : (categoryList.includes('misc') ? 'video_misc' : 'video_compilation')
        );

        const mainCategory = categoriesForFilename; // use first category for filename
        ...
        let bands = [];
        if (lineupBands.length > 0) {
            bands = lineupBands;
        }
        if (bands.length === 0) {
            const bandGuess = title.split('-')[0].trim();
            if (bandGuess && bandGuess.length < 50) {
                bands = [bandGuess];
            }
        }

        const bandForFilename = bands.length > 0 ? cleanFileName(bands[0]) : 'various_artists';

        let partSuffix = '';
        const partMatch = title.match(/\[Part\s*(\d+)\]/i);
        if (partMatch) {
            partSuffix = `_part_${partMatch[1]}`;
        }

        // now safely build locationExtra *after* bandForFilename is set
        let locationExtra = '';
        if (appendLocation) {
            const bandNameClean = bandForFilename;
            let parts = [city, state, country, venue, event]
                .filter(x => x && x.trim() !== '')
                .map(x => cleanFileName(x))
                .filter(p => p !== bandNameClean); // remove band name from location
            if (parts.length > 0) {
                locationExtra = '_' + parts.join('_');
            }
        }

        const filename = cleanFileName(`${bandForFilename}_${datePart}${partSuffix}${locationExtra}${source ? '_' + source.toLowerCase().replace(/\s+/g, '_') : ''}_${mainCategory}`);
        ...
        if (lineupBlockMatch) {   // 🟢 lineupMatches is still defined here!
            ...
            const bandLinks = [...lineupHTML.matchAll(/<a [^>]+>(.*?)<\/a>/gi)];
            for (const link of bandLinks) {
                const linkText = cleanHTML(link[1]);  // 👈 ADD THIS
                const lineupTitle = linkText;   // e.g. Slipknot - 12.06.2005 - Donington - England...

                // Extract band name for filename
                // --- Extract band and date from lineupTitle ---
                const bandName = linkText.split('-')[0].trim();
                ...
                if (/\[compilation\]/i.test(lineupTitle)) {
                    childCategoryList.push('compilation');
                }

                // Always prepend 'video' unless 'audio' is present
                if (!childCategoryList.includes('audio')) {
                    childCategoryList.unshift('video');
                }
                ...
                if (needsLocation) {
                    const afterDatePartRaw = linkText.split('-').slice(1).map(x => x.trim());
                ...
                // Append locationExtra only if needed and doesn't already include band/date
                if (needsLocation && locationExtra) {
                    rawFilename += `${locationExtra}`;
                }

                // Append source suffix (no leading underscore)
                if (sourceSuffix && !rawFilename.includes(sourceSuffix)) {
                    rawFilename += `_${sourceSuffix.replace(/^_+/, '')}`;
                }


                // Append child category if not already in location or source
        ...
        // --- Parse DVD info ---
        const dvdInfoMatch = showContent.match(/DVD info:([\s\S]+?)(Lineup:|Set:|Setlist:|$)/i);
        if (dvdInfoMatch) {
                    ...
                    uniqueLines.forEach(line => {
                        ...
                        if (numMatch) {
                            ...
                            if (parenMatch) {
                                ...
                                // Notes
                                if (/cut|incomplete/i.test(insideParens)) note = "incomplete";
            ...
            function titleToFilename(titleText) {
                ...
                let locationExtra = '';
                if (appendLocation) {
                    const bandNameClean = cleanFileName(bands.length > 0 ? bands[0] : '');
                    let parts = [city, state, country, venue, event]
                        .filter(x => x && x.trim() !== '')
                        .map(x => cleanFileName(x))
                        .filter(p => p !== bandNameClean); // remove band if present
