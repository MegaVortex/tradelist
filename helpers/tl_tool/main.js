...
async function createWindow() {
  ...
  console.log("✅ Renderer loaded:", pagePath);
}
...
app.whenReady().then(async () => {
  try {
    ...
    console.error("⚠ Google Drive authentication is unavailable:", error);
  }
...
app.on("activate", () => {
  if (BrowserWindow.getAllWindows().length === 0) {
    createWindow().catch((error) => console.error("❌ Window creation failed:", error));
  }
});
